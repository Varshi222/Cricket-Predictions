...
for group,frame in India_Bowling.groupby("Match ID"):
    ...
    for g,f in frame.groupby("Bowler"):
        ...
        for i,j in f.groupby("Over type"):
            if (i=="Powerplay"):
                p=0
                for k in range(len(f["Out"])):
                    if(f['Out'].iloc[k]=='lbw' or f['Out'].iloc[k]=='bowled' or f['Out'].iloc[k]=='caught' or 
                   f['Out'].iloc[k]=='caught and bowled' or f['Out'].iloc[k]=='stumped'):
                        p=p+1
...
gbr = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=1)
gbr.fit(X_train, y_train)

pr=gbr.predict(X_test)

print("Accuracy = ",round(metrics.accuracy_score(y_test,pr)*100,2))

pickle.dump(gbr, open('wickets.pkl', 'wb'))
